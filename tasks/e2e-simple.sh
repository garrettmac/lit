...
function create_react_app {
  node "$temp_cli_path"/node_modules/lit/index.js "$@"
}
...
then
  ...
  #    https://github.com/garrettmac/lit/pull/2400
  # When removing workaround, you may run into
  #    https://github.com/garrettmac/lit/issues/2030
  case "$(uname -s)" in
...
# We need to install lit deps to test it
cd "$root_path"/packages/lit
npm install
cd "$root_path"
...
then
  cd $temp_app_path
  err_output=`node "$root_path"/packages/lit/index.js test-node-version 2>&1 > /dev/null || echo ''`
...
./node_modules/.bin/eslint --max-warnings 0 packages/lit/
...
# ******************************************************************************
# First, test the lit development environment.
...
# ******************************************************************************
# Next, pack react-scripts and lit so we can verify they work.
# ******************************************************************************
...
# ******************************************************************************
# Now that we used lit to create an app depending on react-scripts,
